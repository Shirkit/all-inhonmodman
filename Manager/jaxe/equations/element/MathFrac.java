...
public class MathFrac extends MathElement
{
  ...
  public final static String ATTRIBUTE_LINETHICKNESS = "linethickness"; 
    ...
    public void paint(final Graphics g, final int posX, final int posY)
    {
        ...
        e1.paint(g, posX + (width - e1.getWidth(true)) / 2 + 2,
                         middle - e1.getDescentHeight(true) - 1);

        for (int i = 0; i < linethickness; i++)
            g.drawLine(posX + 1, middle + i - linethickness / 2,
                                 posX + getWidth(true) - 1, middle + i - linethickness / 2);

        e2.paint(g, posX + (width - e2.getWidth(true)) / 2 + 2,
   ...
   *                     which has not fixed sizes
    ...
    {
        return Math.max(getMathElement(0).getWidth(dynamicParts), getMathElement(1).getWidth(dynamicParts))
                     + 4;
    }
   ...
   *                     which has not fixed sizes
    ...
    {
        return getMathElement(0).getHeight(true) + 1 + linethickness / 2
                     + getMiddleShift();
    }
   ...
   *                     which has not fixed sizes
    ...
    {
        return Math.max(0, getMathElement(1).getHeight(true) + 1
                                        + linethickness / 2 - getMiddleShift());
    }
}
